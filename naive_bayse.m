...
mdl = fitcnb(data3(c.training,:),class3(c.training));
DTModel = predict(mdl,data3(~c.training,:));
EVAL = Evaluate(class3(~c.training,:)',DTModel');

disp('NB classification dwt results');
...
mdl2 = fitcnb(data4(c.training,:),class4(c.training));
dtModel = predict(mdl2,data4(~c.training,:));
EVAL2 = Evaluate(class4(~c.training,:)',dtModel');

disp('NB classification stat results');
